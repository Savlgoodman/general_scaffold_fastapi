...
@router.get("/user-tree", operation_id="getUserMenuTree", summary="获取���前用户菜单树")
...
async def create_menu(dto: CreateMenuDTO, db: AsyncSession = Depends(get_db)) -> R[None]:
    await menu_service.create_menu(db, dto)
    return R.ok()


@router.put("/{id}", operation_id="updateMenu", summary="更新菜单")
async def update_menu(id: int, dto: UpdateMenuDTO, db: AsyncSession = Depends(get_db)) -> R[None]:
    await menu_service.update_menu(db, id, dto)
    return R.ok()


@router.delete("/{id}", operation_id="deleteMenu", summary="删除菜单")
async def delete_menu(id: int, db: AsyncSession = Depends(get_db)) -> R[None]:
    await menu_service.delete_menu(db, id)
    return R.ok()


@router.put("/sort", operation_id="sortMenus", summary="批量排序菜单")
async def sort_menus(dto: SortMenuDTO, db: AsyncSession = Depends(get_db)) -> R[None]:
